...
int cl_mc_kernel::initialize(const int tid, const vector<float>& h_sf_e, const vector<float>& h_sf_d, const size_t h_sf_ns, const float* h_corner0, const float* h_corner1, const int* h_num_probes, const float h_granularity_inverse, const int num_mc_tasks, const int h_ng, const unsigned long h_seed)
{
...
//	BOOST_SCOPE_EXIT_ALL(&) { free(platforms); free(cSourceString); };
	...
	fclose(pFileStream);
	program = clCreateProgramWithSource(context, 1, (const char **)&cSourceString, &szSourceLength, &error);
	checkOclErrors(error);
	free(cSourceString);
...
//	cout << build << endl;
	kernel = clCreateKernel(program, "mc", &error);
	checkOclErrors(error);
	...
	d_sf_e = clCreateBuffer(context, CL_MEM_READ_ONLY, sf_bytes, NULL, &error);
	checkOclErrors(error);
	d_sf_d = clCreateBuffer(context, CL_MEM_READ_ONLY, sf_bytes, NULL, &error);
	checkOclErrors(error);
	checkOclErrors(clEnqueueWriteBuffer(command_queue, d_sf_e, CL_FALSE, 0, sf_bytes, h_sf_e.data(), 0, NULL, NULL));
	checkOclErrors(clEnqueueWriteBuffer(command_queue, d_sf_d, CL_FALSE, 0, sf_bytes, h_sf_d.data(), 0, NULL, NULL));
...
//	c_sf_e = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(c_sf_e ), &d_sf_e, &error);
...
int cl_mc_kernel::update(const int tid, const vector<vector<float> > h_maps, const vector<size_t>& xs)
{
...
void cl_mc_kernel::launch(vector<float>& h_ex, const vector<int>& h_lig, const int nv, const int nf, const int na, const int np)
{
	...
	cl_mem d_lig = clCreateBuffer(context, CL_MEM_READ_ONLY/* | CL_MEM_COPY_HOST_PTR*/, lig_bytes, NULL/*&h_lig.front()*/, &error);
	checkOclErrors(error);
	...
	cl_mem d_s0 = clCreateBuffer(context, CL_MEM_READ_WRITE, s0_bytes, NULL, &error);
	checkOclErrors(error);
