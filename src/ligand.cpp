	...
	for (g = 0; g < num_generations; ++g)
	{
		...
		while (true)
		{
			...
			for (j = 0; j < num_alphas; ++j)
			{
				...
				x2[1] = x1[1] + alpha * p[1];
				x2[2] = x1[2] + alpha * p[2];
				const array<float, 3> rot3 = alpha * make_array(p[3], p[4], p[5]);
				const float h = norm(rot3) * 0.5f;
				const array<float, 3> axis = (0.5f / h) * rot3;
				assert(normalized(axis));
				const float s = sin(h);
				const float c = cos(h);
				const array<float, 4> rot4 = make_array(c, s * axis[0], s * axis[1], s * axis[2]);
				assert(normalized(rot4));
				const array<float, 4> x1orientation = make_array(x1[3], x1[4], x1[5], x1[6]);
				assert(normalized(x1orientation));
				const array<float, 4> x2orientation = rot4 * x1orientation;
