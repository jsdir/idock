...
public:
	...
	bool evaluate(const conformation& conf, const scoring_function& sf, const box& b, const vector<vector<double>>& grid_maps, const double e_upper_bound, double& e, double& f, change& g) const;
	...
	void write_models(const path& output_ligand_path, const ptr_vector<result>& results, const size_t num_conformations, const box& b, const vector<vector<double>>& grid_maps);
