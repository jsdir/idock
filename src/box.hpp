...
public:
	...
	box(const vec3& center, const vec3& span_, const double grid_granularity);
	...
	double project_distance_sqr(const vec3& corner1, const vec3& corner2, const vec3& coordinate) const;

	/// Returns true if the distance between a coordinate and the surface of current box is within cutoff.
	double project_distance_sqr(const vec3& coordinate) const;
