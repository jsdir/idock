...
void scoring_function::score5(float* const v, const size_t t1, const size_t t2, const double r2)
{
	BOOST_ASSERT(r2 <= Cutoff_Sqr);

	// Calculate the surface distance d.
	const double d = sqrt(r2) - (xs_vdw_radius(t1) + xs_vdw_radius(t2));
	...
	v[2] += (d > 0 ? 0.0 : d * d);
	v[3] += ((xs_is_hydrophobic(t1) && xs_is_hydrophobic(t2)) ? ((d >= 1.5) ? 0.0 : ((d <= 0.5) ? 1.0 : 1.5 - d)) : 0.0);
	v[4] += ((xs_hbond(t1, t2)) ? ((d >= 0) ? 0.0 : ((d <= -0.7) ? 1 : d * (-1.428571))): 0.0);
}
